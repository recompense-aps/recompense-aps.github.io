...
function Draw4(image, x, y, angle) { 
...
function Draw5(image, x, y, angle, alpha){
...
function Draw6(image, rectangle, angle){
	canvas.save(); 
	...
	// draw it up and to the left by half the width
	// and height of the image 
	canvas.drawImage(image, -(image.width/2), -(image.height/2), rectangle.width, rectangle.height);
 
	// and restore the co-ords to how they were when we began
	canvas.restore(); 
}
function Draw7(image, x, y, alpha) {
	canvas.globalAlpha = alpha;
